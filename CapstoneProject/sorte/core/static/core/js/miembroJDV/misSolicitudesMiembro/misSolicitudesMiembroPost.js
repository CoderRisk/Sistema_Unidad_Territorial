...
function postulaciones(){
    ...
    solicitudPostulacion.forEach(postulacion => {
        ...
        let postulacionDisplay = window.getComputedStyle(postulacion);

        if(postulacionDisplay.display === '' && linkPostulacion.style.backgroundColor === ''){
            postulacion.style.display = "block";
            linkPostulacion.style.backgroundColor = '#3d8af7';

        }else if(postulacionDisplay.display === 'block'){
